...
5. **Google連携機能**
   ...
   - **Google Drive ファイルアップロード**:
     ...
     - **柔軟なフォルダ指定**:
       - コンテンツフォルダIDを設定した場合: 指定フォルダに直接アップロード
       - コンテンツフォルダIDを設定していない場合: Spreadsheetと同じフォルダ内の`files`サブフォルダに保存
     - 共有リンク生成（リンクを知っている人全員が閲覧可能）
     - ファイルURLをSpreadsheetに記録
   - **設定項目**:
     - Google Sign-In（Sheets & Drive API権限）
     - Spreadsheet ID設定（手動入力）
     - コンテンツフォルダID設定（任意、手動入力またはマスタースプレッドシートから選択）
   - **マスタースプレッドシート機能**:
     - 複数のSpreadsheet設定を一元管理
     - 設定項目: ID、名前、備考、コンテンツフォルダID
     - アプリから設定を選択して入力欄に自動設定
     - 「保存」ボタンで確定
